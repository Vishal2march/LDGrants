...
<script type="text/javascript">
    $(document).ready(function () {
        $('#ContractedServicesTableContainer').jtable({
            title: 'Family Literacy - Purchased Services Year 1',
            footer: true,
            actions: {
                listAction: 'literacyYr1ContractedServicesList.action',
                createAction: 'addUpdateLiteracyContractedServicesRow.action',
                //updateAction is supposed to only return "result", but reusing addupdate action seems to work ok
                updateAction: 'addUpdateLiteracyContractedServicesRow.action',
            ...
            recordsLoaded: function(event, data) {
			  ...
			  if (myDisable=='true'){
			     $('#ContractedServicesTableContainer').find('.jtable-command-button.jtable-edit-command-button').remove();
			  	 $('#ContractedServicesTableContainer').find('.jtable-command-button.jtable-delete-command-button').remove();
			     $('#ContractedServicesTableContainer').find('.jtable-toolbar-item.jtable-toolbar-item-add-record').remove();
			  }
			}
        });
