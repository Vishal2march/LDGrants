...
<script type="text/javascript">
    $(document).ready(function () {
        $('#EquipmentTableContainer').jtable({
            title: 'Family Literacy - Equipment Year 1',
            footer: true,
            actions: {
                listAction: 'literacyYr1EquipmentList.action',
                createAction: 'addUpdateLiteracyEquipmentRow.action',
                //updateAction is supposed to only return "result", but reusing addupdate action seems to work ok
                updateAction: 'addUpdateLiteracyEquipmentRow.action',
                deleteAction: 'deleteLiteracyEquipmentRow.action'
            },
            fields: {            
                id: {
                    key: true,
            ...
            recordsLoaded: function(event, data) {			  
			  ...
			  if (myDisable=='true'){
			     $('#EquipmentTableContainer').find('.jtable-command-button.jtable-edit-command-button').remove();
			  	 $('#EquipmentTableContainer').find('.jtable-command-button.jtable-delete-command-button').remove();
			     $('#EquipmentTableContainer').find('.jtable-toolbar-item.jtable-toolbar-item-add-record').remove();
			  }
			}
        });
                
        $('#EquipmentTableContainer').jtable('load'); 
        
        //set the active bootstrap pill (the budget tab button for equip)
...
<div class="panel panel-default">
  ...
  <br/><br/>
  <b>Equipment Initial Application Total: $<s:property value="%{getText('{0,number,integer}', {initialCategoryTotal})}"/></b>
