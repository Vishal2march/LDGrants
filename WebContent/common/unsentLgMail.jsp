...
<html>
  ...
  <c:when test="${param.i=='delete'}">
    ...
    <form action="adminEmailNav.do?item=deletewt" method="POST">
  ...
  <table width="100%" summary="for layout only">
    ...
    <c:forEach var="row" items="${wtBeans}" >     
      <c:url var="wtupdate" value="adminEmailNav.do">
        <c:param name="item" value="viewtemplate" />
        <c:param name="id" value="${row.id}" />
        <c:param name="pr" value="lg"/>
      </c:url>
      <c:url var="wtdelete" value="adminEmailNav.do">
        <c:param name="item" value="confirmdelete" />
        <c:param name="id" value="${row.id}" />
        <c:param name="pr" value="lg"/>
      </c:url>      
      <c:url var="wtsend" value="adminEmailNav.do">
        <c:param name="item" value="confirmsend" />
        <c:param name="id" value="${row.id}" />
        <c:param name="prog" value="lg"/>
      </c:url>
        <tr>
