 ...
 * Name of the Application        :  fs10fHTML.jsp
 * Creation/Modification History  :
 *
 *     SHusak       8/31/07     Created
 *
 * Description
 * This page is an html read only view of the FS10F form. used for lg/fl/al/di
 * coord uses fs10f long form
--%>
...
<html>
 ...
 <c:choose>
 <c:when test="${thisGrant.fccode==80}"><%--lgrmif--%>
    <c:set var="fcpre" value="05"/>
    <c:set var="sdate" value="7/1/"/>
    <c:set var="edate" value="6/30/"/>
 </c:when><%--cp disc, al/fl--%>
 <c:when test="${thisGrant.fccode==5 || thisGrant.fccode==40 || thisGrant.fccode==42}">
    <c:set var="fcpre" value="03"/>
    <c:set var="sdate" value="7/1/"/>
    <c:set var="edate" value="6/30/"/>
 </c:when>
 <c:when test="${thisGrant.fccode==6}"><%--cp statutory--%>
    <c:set var="fcpre" value="03"/>
  ...
  <table align="center" width="96%" summary="for layout only">
    <tr>
      ...
      <td valign="top"><b>FINAL EXPENDITURE REPORT FOR A
  ...
  <table align="center" class="fsLarge" summary="for layout only" >
    ...
    <tr>
      <td>     
        <p>Agencies should use the FS-10-F Short Form unless directed otherwise in the grant 
        ...
        dates.</p>
        <p>For State projects, final expenditure reports are due within 30 days after the 
        ...
        verify the due date.</p>
        <p>Agencies must record grant expenditure details in a manner consistent with the 
        ...
        the Department, the Office of the State Comptroller and federal agencies.</p>
        <p>Submit three signed original sets of the FS-10-F Short Form as a two page 
        ...
        program for which you are applying. </p>               
        <p>For Special Legislative Projects, submit one report with original signature
        and two copies, along with a final program narrative report.</p>
        ...
        <p>For further information about completing the final expenditure report, please 
        refer to the Federal and State Aided Grants at www.oms.nysed.gov/cafe/ or 
        contact Grants Finance at grantsweb@nysed.gov or (518) 474-4815.</p>        
      </td>
    </tr>
  </table>
